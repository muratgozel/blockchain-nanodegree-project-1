...
class Blockchain {
    ...
    requestMessageOwnershipVerification(address) {
        return new Promise((resolve) => {
          const currentTimeStr = new Date().getTime().toString().slice(0,-3)
          const message = address + ':' + currentTimeStr + ':starRegistry';
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
          ...
          if ((currentTime - messageTime) > 300) {
            return reject(new Error('Transaction timeout.'));
          }

          const verified = bitcoinMessage.verify(message, address, signature);
          if (verified !== true) {
            return reject(new Error('Invalid transaction signature.'));
          }

          const block = new BlockClass.Block({data: star});
          self._addBlock(block).then(function(validBlock) {
            return resolve(validBlock);
          });
        });
    }
    ...
    getBlockByHash(hash) {
        ...
        return new Promise((resolve, reject) => {
          ...
          return resolve(block ? block : null);
        });
    }
    ...
    getStarsByWalletAddress (address) {
        ...
        let stars = [];
        return new Promise((resolve, reject) => {
          const promises = self.chain.map(function(block) {
            return block.getBData();
          });
          Promise.all(promises).then(function(results) {
            results.map(r => r ? stars.push({owner: address, star: r}) : null);
